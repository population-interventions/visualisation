...
module.exports = function (eleventyConfig) {
    ...
    eleventyConfig.addFilter("get_skip_to_links", function (post) {
        ...
        let html =
            `<ul class="jump-navigation fixed headless skip-to">` +
            `<li>Skip to</li>`;
        ...
        for (let link of post.data.skipToLinks) {
            ...
            html +=
                ...
                link.href +
                `" ${attrs}>` +
                `<div class="img-wrap"><img src="/posts/` +
                post.data.slug +
                `/` +
                link.image +
                `" /></div>` +
    ...
    eleventyConfig.addFilter("get_graph_spec", function (name, slug) {
        let spec = JSON.parse(
                fs
                    .readFileSync(`../src/posts/${slug}/graph-${name}.json`)
                    .toString()
            ),
            uom = {};
    ...
    eleventyConfig.addShortcode("import_content", function (name, slug) {
        let html = fs
            .readFileSync(`../src/posts/${slug}/${name}.html`)
            .toString();
    ...
    // Add shortcode to import images.
    eleventyConfig.addShortcode(
        "import_image_link",
        function (name, url, slug) {
            return `<a class="image-link" href="${url}" target="_blank"><img src="/posts/${slug}/${name}" /></a>`;
        }
    );
    ...
    eleventyConfig.addShortcode("import_graph", function (name, slug) {
        ...
        for (let toggle_group of toggle_specs) {
            ...
            for (let toggle of toggle_group.toggles) {
                ...
                if (toggle.type == "range") {
                    ...
                    toggle_group_contents += `<select name="${
                        toggle.signalName
                    }" id="select-${name}-${
                        toggle.signalName
                    }" autocomplete=off>
                ...
                if (toggle.hasOwnProperty("detailLink")) {
                    toggle_group_contents +=
                        `<a href="` +
                        toggle.detailLink +
                        `" class="detail-link" target="_blank" rel="noopener noreferrer"><span data-icon="info-outline" class="smaller"></span> <span>Details</span></a>`;
                }
        ...
        let figPosition = uom.hasOwnProperty("figPosition")
                ? uom.figPosition
                : "inset",
            increaseMargin = uom.hasOwnProperty("increaseMargin")
                ? uom.increaseMargin
                : 1;
    ...
    eleventyConfig.addFilter("groupBySection", function (collection) {
        let rval = [],
            sections = [
                ...new Set(collection.map((post) => post.data.section)),
            ];
    ...
    eleventyConfig.addFilter("footnotes_to_sidenotes", function (html) {
        ...
        let regex =
            /<li id="fn\d+" class="footnote-item"><p>((.|\n)*?)<\/p>\n<\/li>/g;
        ...
        footnotes = footnotes.map(function (fn) {
            let idx = fn.match(/#fnref(\d+)/g)[0].replace(/#fnref/g, ""),
                text = fn.replace(
                    / <a href="#fnref\d+" class="footnote-backref">↩︎<\/a>/g,
                    ""
                );
            side = text[0];
